        ...
        async for doc in cursor:
            ...
            for field in SCHOOL_EXPORT_FIELDS:
                if field == "tags":
                    ...
                    # legacy CSV importer used to write, before it was deleted.
                    # This fallback still serves rows the Task 4 migration hasn't
                    # touched yet.
...
async def run_auto_reminders():
    while True:
        try:
            ...
            # actually delivers messages) but ran every 60s vs. the real
            # executor's hourly cadence, so it nearly always reached a due
            # step first and "consumed" it: for a WhatsApp step it only
            # inserted a whatsapp_logs row (status: "queued", never sent) and
            # advanced current_step/next_step_at as if the step had gone out.
            # In production this produced 131 stuck-"queued" drip WhatsApp
            # logs and marked sequences complete without ever sending
            # anything. scheduler.py remains the only drip executor now.
            ...
            logging.info(
                ...
                f"{greetings_queued} greetings queued, "
