...
API.interceptors.response.use((res) => {
  ...
  if (method && ['post', 'put', 'delete', 'patch'].includes(method)) {
    ...
    else if (/\/mail-(runs|areas)/.test(url))      domain = 'mail';   // before crm: /mail-runs/.../sync-schools contains "schools"
    else if (/\/(orders|dispatches)/.test(url))    domain = 'orders';
...
export const crmMaintenance = {
  ...
  exportDuplicatesXlsx: () => API.get('/crm/maintenance/duplicate-schools/export.xlsx', { responseType: 'blob' }),
  applyDuplicateReview: (formData) => API.post('/crm/maintenance/duplicate-schools/apply', formData, { headers: { 'Content-Type': 'multipart/form-data' } }),
  mergeSchools: (data) => API.post('/crm/maintenance/schools/merge', data),
  mergeLog: () => API.get('/crm/maintenance/schools/merge-log'),
  undoMerge: (id) => API.post(`/crm/maintenance/schools/merge/${id}/undo`),
...
export const engagement = {
  ...
  drill: (metric, value = '', days = 30, extra = {}) => API.get('/engagement/drill', { params: { metric, value, days, ...extra } }),
};
...
export const certsApi = {
  ...
  clearGenerated:   (id)            => API.delete(`/certs/batches/${id}/generated`),
  deleteBatch:      (id)            => API.delete(`/certs/batches/${id}`),
