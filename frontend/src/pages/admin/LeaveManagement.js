...
export default function LeaveManagement() {
  ...
  const Layout      = isSalesUser ? SalesLayout : AdminLayout;
  const layoutProps = isSalesUser ? { title: 'Leave Management', showBack: true } : {};
          ...
          <div>
            ...
            <p className={`${textSec} mt-1 text-sm`}>
              {canApprove
                ...
                : `${myLeaves.filter(l => l.status === 'approved').length} approved, ${myLeaves.filter(l => l.status === 'pending').length} pending`}
            </p>
          </div>
          ...
          {['all', 'pending', 'approved', 'rejected'].map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all capitalize ${
                filter === f ? 'bg-[#e94560] text-white' : `${card} border ${textSec} ${hoverBg}`
              ...
              <div key={lv.leave_id} className={`${card} border rounded-lg p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3`} data-testid={`leave-${lv.leave_id}`}>
                <div className="flex items-start gap-3 flex-1">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {lv.approved_by && (
                      <p className={`text-xs ${lv.status === 'approved' ? 'text-green-400' : 'text-red-400'} mt-1`}>
                        {lv.status === 'approved' ? 'Approved' : 'Rejected'} by {lv.approved_by}{lv.remarks ? ` — ${lv.remarks}` : ''}
                      </p>
                    )}
                  </div>
                </div>
          ...
          <DialogContent className={`${dlgCls} w-[calc(100vw-1rem)] sm:max-w-md max-h-[88dvh] overflow-y-auto`}>
            ...
            <div className="space-y-4 py-2">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Input type="date" value={form.from_date}
                    onChange={e => setForm({...form, from_date: e.target.value, to_date: form.to_date || e.target.value})}
                    className={inputCls} data-testid="leave-from-date" />
                </div>
                <div>
                  <Label className={`${textSec} text-xs`}>To Date *</Label>
                  <Input type="date" value={form.to_date}
                    onChange={e => setForm({...form, to_date: e.target.value})}
                    className={inputCls} min={form.from_date} data-testid="leave-to-date" />
                </div>
              </div>
              <div className="flex items-center justify-between">
                ...
                {formDays > 0 && (
                  <span className={`text-sm font-medium ${textPri}`}>{formDays} day{formDays !== 1 ? 's' : ''}</span>
                )}
              </div>
              <div>
                <Label className={`${textSec} text-xs`}>Reason</Label>
                <Input value={form.reason} onChange={e => setForm({...form, reason: e.target.value})}
                  className={inputCls} placeholder="Why do you need leave?" data-testid="leave-reason" />
              </div>
            </div>
            <DialogFooter>
