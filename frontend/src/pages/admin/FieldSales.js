...
import { MapPin, Calendar, Users, Route, Clock, CheckCircle, TrendingUp, Navigation, Download, AlertTriangle, LogIn, LogOut, BarChart2, ChevronDown, ChevronUp, Target, Save } from 'lucide-react';
...
export default function FieldSales() {
  ...
  const [targetRows, setTargetRows]   = useState({});  // email → {visits_target, leads_target, demos_target}
  ...
  useEffect(() => { if (activeTab === 'targets') loadTargets(targetMonth); }, [activeTab, targetMonth]);
      ...
      <div className="space-y-8">
        ...
        {activeTab === 'targets' && (
          <div>
            ...
            {salesReps.length === 0 ? (
              <div className="text-center py-16 text-[var(--text-muted)]">No sales reps found. Add users with the "sales" role in User Management.</div>
            ) : (
