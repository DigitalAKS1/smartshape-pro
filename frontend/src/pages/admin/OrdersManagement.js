...
import { Package, Truck, CheckCircle, Clock, XCircle, Search, Eye, ArrowRight, Lock, Unlock, ShieldCheck, AlertTriangle, MessageSquare, CreditCard, DollarSign } from 'lucide-react';
              ...
              <div className={`${card} border rounded-md overflow-hidden`}>
                ...
                <div className="hidden md:block overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {holdsList.map(h => (
                        <tr key={h.order_item_id} className="border-t border-[var(--border-color)] hover:bg-[var(--bg-hover)]" data-testid={`hold-row-${h.order_item_id}`}>
                ...
                <div className="md:hidden divide-y divide-[var(--border-color)]">
                  {holdsList.map(h => (
                    <div key={h.order_item_id} className="p-4 space-y-2" data-testid={`hold-card-${h.order_item_id}`}>
                      <div className="flex items-center justify-between">
                        <div>
                        ...
                        {h.available < 0 && <AlertTriangle className="h-4 w-4 text-red-400" />}
                      </div>
