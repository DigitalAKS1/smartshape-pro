...
export default function EditQuotation() {
  ...
  const calcTotals = () => {
    ...
    const disc1 = subtotal * (d1 / 100);
    const after1 = subtotal - disc1;
    const disc2 = after1 * (d2 / 100);
    const subtotal_after_disc = after1 - disc2;
    const total_gst = subtotal_after_disc * 0.18;
    const freight_gst = fr * 0.18;
    const freight_with_gst = fr + freight_gst;
      ...
      <div className="max-w-5xl mx-auto space-y-6">
          ...
          <div className="border border-[var(--border-color)] rounded-md overflow-hidden">
            <table className="w-full text-sm">
              <thead><tr className="bg-[var(--bg-primary)]">
                ...
                <th className="text-center text-xs text-[var(--text-muted)] py-2 px-3 w-20">GST %</th>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-[var(--bg-primary)] border border-[var(--border-color)] rounded-md p-5 space-y-2">
            ...
            <div className="flex justify-between text-sm"><span className="text-[var(--text-secondary)]">Items Total</span><span className="font-mono text-[var(--text-primary)] font-semibold">{formatCurrency(t.subtotal)}</span></div>
            {(quot.discount1_pct || 0) > 0 && <div className="flex justify-between text-sm text-green-400"><span>Discount ({quot.discount1_pct}%)</span><span className="font-mono">−{formatCurrency(t.disc1)}</span></div>}
            {(quot.discount2_pct || 0) > 0 && <div className="flex justify-between text-sm text-green-400"><span>Additional Discount ({quot.discount2_pct}%)</span><span className="font-mono">−{formatCurrency(t.disc2)}</span></div>}
            {((quot.discount1_pct || 0) > 0 || (quot.discount2_pct || 0) > 0) && (
              <div className="flex justify-between text-sm border-t border-[var(--border-color)] pt-1 font-semibold"><span className="text-[var(--text-primary)]">Subtotal After Discounts</span><span className="font-mono text-[var(--text-primary)]">{formatCurrency(t.subtotal_after_disc)}</span></div>
            )}
            <div className="flex justify-between text-sm"><span className="text-[var(--text-secondary)]">Total GST @ 18%</span><span className="font-mono text-[var(--text-secondary)]">{formatCurrency(t.total_gst)}</span></div>
            {(quot.freight_amount || 0) > 0 && <div className="flex justify-between text-sm"><span className="text-[var(--text-secondary)]">Freight incl. 18% GST</span><span className="font-mono text-[var(--text-secondary)]">{formatCurrency(t.freight_with_gst)}</span></div>}
