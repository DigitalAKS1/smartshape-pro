...
**Expected:** the webhook URL is `.../api/webhooks/whatsapp/smartshape` (a `?t=` suffix means it was registered before the header existed) with all five events `true`; the `wget` line prints an HTTP status line (e.g. `HTTP/1.1 422`) — this is the **hairpin check**: Evolution calls the public `https://app.smartshape.in` hostname from *inside* the same docker host, which some networks/NAT setups block. **If this fails →** (a `?t=` suffix, or `log_format` is absent on the VPS, meaning the default `combined` format is logging the query string too): rotate the secret and re-register every instance without a relink — `setbe WA_WEBHOOK_SECRET "$(openssl rand -hex 24)"` → recreate the backend → then, as an admin, `curl -s -c /tmp/wa.cookies -H 'Content-Type: application/json' -d '{"email":"info@smartshape.in","password":"<owner types this>"}' https://app.smartshape.in/api/auth/login >/dev/null && curl -s -b /tmp/wa.cookies -X POST https://app.smartshape.in/api/wa/instances/rewebhook-all | python3 -m json.tool; rm -f /tmp/wa.cookies`. **If the hairpin `wget` fails/times out →** add the host's public IP to the compose file's `evolution-api` service on the VPS: `extra_hosts: ["app.smartshape.in:<VPS_PUBLIC_IP>"]`, then `docker compose -p "$EVO_PROJECT" -f docker-compose.evolution.yml up -d evolution-api` to apply it, and re-run this step.
